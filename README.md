...
Учебный MVP для ВКР: клиент-серверное веб-приложение, где пользователь создаёт задачу, задаёт критерии, альтернативы и значения, запускает методы оптимизации и смотрит результаты в таблицах и графиках.
...
## Частые проблемы Docker на Windows

Если `docker info` пишет `Docker Desktop is unable to start`, чаще всего не готов WSL2 backend.
...
Если список дистрибутивов пустой или WSL не установлен, выполните PowerShell от имени администратора:

```powershell
wsl --install
```

После установки перезагрузите Windows, запустите Docker Desktop и повторите:
...
Если появляется предупреждение `Error loading config file ... .docker\config.json: Access is denied`, проверьте права на файл `C:\Users\Александр\.docker\config.json` или временно переименуйте его после закрытия Docker Desktop.
